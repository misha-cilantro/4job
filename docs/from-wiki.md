...
**Nothing in this file is this project's own invention.** It records only what
the sources below say. Everything this project adds, changes or assumes lives in
[customizations.md](customizations.md) — keep it that way, so this file stays
usable as the authority the data is checked against.
...
Verbatim, for the three that matter most:
...
Things the pages above don't settle. What the project decided to do about each is
in [customizations.md](customizations.md).

- **Which crystal a Fifth Job comes from.** Never stated.
- **Who chooses the job Forbidden crosses out, and when.** The rule says one job
  is crossed out on entering the Void, but not by whom.
- **When the Advance jobs become available.** Extra Jobs names the three jobs but
  gives no timing.
- **Whether Classic enables duplicates.** Only Team 375 is documented to.
- **"Pure Chaos."** The Freelancer page names it as a run type that can assign
  nonstandard jobs, and Regchaos was retired in favour of "Volcano with a
  Duplicates clause", but neither "Pure" nor "Chaos" appears on the current
  help.php run type list. There may be run types, or older names for existing
  ones, these pages don't cover. Resolving it needs a source not found here.
